...
/**
 * Nombre de Archivo: EstadoDonante.class.php
...
class EstadoDonante extends Conexion {
    ...
    public function insert_EstadoDonante() {
        try {
            ...
            $sql = "INSERT INTO estado_donante VALUES(:id_est_donante,:estado_donante,:activo)";
    ...
    public function update_EstadoDonante($arrayCampos, $arrayValue, $arrayWhere) {
        try {
            $this->conection->beginTransaction();
            $where = "";
            $campos = "";
            ...
            foreach ($arrayWhere as $key => $value):
                ...
                $cont++;
            endforeach;
            $sql = "UPDATE estado_donante SET " . $campos . " WHERE " . $where;
            $resultSet = $this->conection->prepare($sql);
    ...
    public function delete_EstadoDonante($arrayValue, $arrayWhere) {
        try {
            $this->conection->beginTransaction();
            $where = "";
            //RECORRIENDO LAS CONDICIONES PARA LA ACTUALIZACION
            ...
            foreach ($arrayWhere as $key => $value):
                ...
                $cont++;
            endforeach;
            $sql = "DELETE FROM estado_donante WHERE " . $where;
            $resultSet = $this->conection->prepare($sql);
            //RECORRIENDO LOS NUEVOS VALORES
            foreach ($arrayValue as $key => &$value):
                $resultSet->bindParam($key, $value);
    ...
    public function select_EstadoDonante($all_row = true) {
        try {
            $array_data = array();
            //verificacion si se filtraran los datos
            if ($all_row == true):
                $this->sqlQuery = "SELECT * FROM estado_donante ORDER BY id_est_donante ASC";
                $resultSet = $this->conection->prepare($this->sqlQuery);
            else:
                $this->sqlQuery = "SELECT * FROM estado_donante WHERE id_est_donante=:id_est_donante";
                $resultSet = $this->conection->prepare($this->sqlQuery);
                $resultSet->bindParam(":id_est_donante", $this->id_est_donante);
            endif;
            $resultSet->execute();
            ...
            if ($coicidencias > 0) {
                ...
                while ($row = $resultSet->fetch(PDO::FETCH_ASSOC)) {
                    $array_data[$i] = array(
                        "id_est_donante" => $row["id_est_donante"],
                        "estado_donante" => $row["estado_donante"],
                        "activo" => $row["activo"]
                    );
                    $i++;
                }
