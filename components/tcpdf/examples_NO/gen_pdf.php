...
function getNullTable($database,$week,$year)
{
	...
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='ES SEW' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
	$sew = mssql_num_rows($result);
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='FYZ' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
	$fyz = mssql_num_rows($result);
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='CCC' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
	$ccc = mssql_num_rows($result);
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='CDV' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
	$cdv = mssql_num_rows($result);
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='PEDREGAL ACT' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
	$act = mssql_num_rows($result);
	$result = $database -> database_query ("select * from opr.opr_wip where wip_planta='PEDREGAL NEWWEAR' AND wip_year=$year AND wip_week=$week AND (wip_lots != 0 OR wip_goal != 0 OR (wip_wo_multisize != '' AND wip_wo_multisize != 0) OR wip_wip1 != 0 OR wip_wip2 != 0 OR wip_wip3 != 0 OR wip_goal_str != ''  OR wip_actual_lots != 0)");
...
function getMonthTodate($planta,$year,$week)
{
...
function getBwopMTD($planta,$year,$week)
{
...
// set the starting point for the page content
$pdf->setPageMark();
$pdf->SetMargins(10, 20, PDF_MARGIN_RIGHT);
...
$execQuery = "
		...
		if($bandMTD == 1)
		$table1 .= '<td colspan="2" align="center"><b>WK Schedule</b></td>';
		
		if($bandMTD == 0)
			$table1 .= '<td colspan="2" align="center"><b>WK Latest Plan</b></td>';
			
		$table1 .= '<td colspan="2" align="center"><b>WK Actual</b></td>';
		
		
		
		// $table1 .= '<td colspan="2" align="center"><b>B/W vs Latest Plan</b></td>';
		$table1 .= '<td colspan="2" align="center"><b>WK '.$label.'</b></td>';
		$table1 .= '<td width="68px" rowspan="2" align="center"><b>Efficiency OPC</b></td>';
		$table1 .= '<td width="68px" rowspan="2" align="center"><b>Efficiency Real</b></td>';
		
		
		
		
		//NEW CHANGE
		$table1 .= '<td colspan="2" style="background-color:#FFFF7C;"  align="center"><b>ACTUAL MTD</b></td>';
		
		$table1 .= '<td colspan="2" style="background-color:#FFFF7C;"  align="center"><b>MTD '.$label.'</b></td>';
		...
		while($row = $database -> database_array($result))
		{
			...
			else
			{
				...
				$table1 .= '<tr>';
					...
					//NEW CHANGE
					$mont=getMonthTodate($row[11],$year,$week);
					$table1 .= '<td '.$font.' align="right">'.FormatNum($mont[0]).'</td>';
					$table1 .= '<td '.$font.' align="right">'.FormatNum($mont[1]).'</td>';
					
					
					//NEW CHANGE
					$bwop=getBwopMTD($row[11],$year,$week);
					// $table1 .= '<td '.$font.' align="right">'.FormatNum($bwop[0]).'</td>';
					// $table1 .= '<td '.$font.' align="right">'.FormatNum($bwop[1]).'</td>';
					$table1 .= '<td '.$font.' align="right">'.FormatNum($mont[0]-$bwop[0]).'</td>';
					$table1 .= '<td '.$font.' align="right">'.FormatNum($mont[1]-$bwop[1]).'</td>';
				...
				if($row[11] == 1 || $row[11] == 2){
					...
					$sumaBWOPTextiles1 += ($mont[0]-$bwop[0]);
					$sumaBWOPTextiles2 += ($mont[1]-$bwop[1]);
					...
					$sumaBWOPInternal1 += ($mont[0]-$bwop[0]);
					$sumaBWOPInternal2 += ($mont[1]-$bwop[1]);
				}
				else if($row[11] == 7 || $row[11] == 8 || $row[11] == 9){
					$sumaContract1 += $mont[0];
					$sumaContract2 += $mont[1];
					
					$sumaBWOPContract1 += ($mont[0]-$bwop[0]);
					$sumaBWOPContract2 += ($mont[1]-$bwop[1]);
...
SELECT 
		...
		{		
			$table1 .= '<tr>';
				...
				//EFFICENCE
				$table1 .= '<td width="68px" align="center"></td>';
				$table1 .= '<td width="68px" align="center"></td>';
				...
				//IMPRIME TOTALES DE MTD B/W vrs ....
				$table1 .= '<td '.$font.' align="right">'.FormatNum($row[13]).'</td>';
				$table1 .= '<td '.$font.' align="right">'.FormatNum($row[14]).'</td>';
...
{		
	...
	if(strlen($comm1) > 1)
	{	
		$table1 .= '<tr><td width="950px"><img src="../../../images/icons/blue_arrow.gif">
