...
function doOnLoad() {
    ...
    toolbar.loadXML("../../../components/toolbar/Mantenimiento_grado.xml?etc=" + new Date().getTime());
...
function LoadGrid()
{
    ...
    mygrid.loadXML("../actions/grado_grid.php",
        function()
        {
...
function SaveData()
{
    ...
    try
    {
        ...
        if(CheckParam())
        {
            ...
            var valor=0;
            var parameters="?p0="+document.getElementById('txtGrado').value;            
            ...
            if (idReg==0)
            {
                ...
                loader = dhtmlxAjax.post( "../actions/grado_insert.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
            else
            {
                ...
                parameters+="&p2="+idReg;                
                loader = dhtmlxAjax.post( "../actions/grado_update.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
...
{
    var Val = true
    var dato=document.getElementById("txtGrado").value;
    if(dato==""){
        Val=false;
...
function LoadParam()
{
    ...
    // llenamos las cajas
    idReg = mygrid.cells(mygrid.getSelectedId(),0).getValue();
    document.getElementById('txtGrado').value=mygrid.cells(mygrid.getSelectedId(),1).getValue();
...
function DeleteData()
{
    ...
    if (idReg>0)
    {
        loader = dhtmlxAjax.post( "../actions/grado_delete.php",encodeURI(parameters), function(){
            ReadXml()
        } );
    }
