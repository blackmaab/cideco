...
function doOnLoad() {
    ...
    toolbar.loadXML("../../../components/toolbar/Mantenimiento_estado_donacion.xml?etc=" + new Date().getTime());
...
function LoadGrid()
{
    ...
    mygrid.loadXML("../actions/estado_donacion_grid.php",
        function()
        {
    ...
    try
    {
        ...
        if(CheckParam())
        {
            ...
            if (idReg==0)
            {
                ...
                loader = dhtmlxAjax.post( "../actions/estado_donacion_insert.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
            else
            {
                ...
                parameters+="&p2="+idReg;                
                loader = dhtmlxAjax.post( "../actions/estado_donacion_update.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
...
{
    var Val = true
    var dato=document.getElementById("txtDescripcion").value;
    if(dato==""){
        Val=false;
...
function LoadParam()
{
    ...
    // llenamos las cajas
    idReg = mygrid.cells(mygrid.getSelectedId(),0).getValue();
    document.getElementById('txtDescripcion').value=mygrid.cells(mygrid.getSelectedId(),1).getValue();
...
function DeleteData()
{
    ...
    if (idReg>0)
    {
        loader = dhtmlxAjax.post( "../actions/estado_donacion_delete.php",encodeURI(parameters), function(){
            ReadXml()
        } );
    }
