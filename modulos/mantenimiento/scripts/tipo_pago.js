...
function SaveData()
{
    ...
    try
    {
        ...
        if(CheckParam())
        {
            ...
            parameters+="&p1="+document.getElementById('txtMeses').value; 
            ...
            if (idReg==0)
            {
                ...
                loader = dhtmlxAjax.post( "../actions/tipo_pago_insert.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
            else
            {
                ...
                parameters+="&p3="+idReg;
                loader = dhtmlxAjax.post( "../actions/tipo_pago_update.php",encodeURI(parameters), function(){
                    ReadXml()
                } );
            }
...
function CheckParam()
{
    ...
    if(meses!=""){    
        patron=/^[0-9]{1,}$/i;
        if(!patron.test(meses)){
            mensaje_error="En el campo: Meses solo se permiten numeros";
            Val=false;
        }
    }
...
function LoadParam()
{
    ...
    document.getElementById('txtMeses').value=mygrid.cells(mygrid.getSelectedId(),2).getValue();
    if(mygrid.cells(mygrid.getSelectedId(),3).getValue()=="Si"){
