...
function LoadAlumnos()
{
	...
	loader = dhtmlxAjax.post( "../actions/beca_get_alumno.php",encodeURI(parameters), function(){ReadXml()} );
...
function LoadBeca()
{
	...
	loader = dhtmlxAjax.post( "../actions/beca_get_beca.php",encodeURI(parameters), function(){ReadXml()} );
	...
	try
	{
		...
		if(CheckParam())
		{
			...
			if (idReg==0)
			{
				...
				loader = dhtmlxAjax.post( "../actions/beca_alumno_save.php",encodeURI(parameters), function(){ReadXml()} );
				MsjWait.show();
			...
			else
			{
				...
				loader = dhtmlxAjax.post( "../actions/beca_alumno_update.php",encodeURI(parameters), function(){ReadXml()} );
				MsjWait.show();
...
function SaveBeca()
{
	...
	try
	{
		...
		if(CheckParam())
		{
			...
			{
				// si es nuevo entra aqui
				loader = dhtmlxAjax.post( "../actions/beca_escolar_save.php",encodeURI(parameters), function(){ReadXml()} );
				MsjWait.show();
			}
			else
			{
				// si es un update entra aqui
				loader = dhtmlxAjax.post( "../actions/beca_escolar_update.php",encodeURI(parameters), function(){ReadXml()} );
				MsjWait.show();
			}
...
var RegNew_Submit = function() { SaveBeca(); };
var RegNew_Cancel = function() { document.getElementById('frm_hide').click();};	

var RegDel_Submit = function() { DeleteData(); };
var RegDel_Cancel = function() { document.getElementById('msj_hide').click();};	


var AluNew_Submit = function() { SaveAlumno(); };
var AluNew_Cancel = function() { document.getElementById('Alu_hide').click();};	
...
function init() {
	...
	try
	{
		...
		MsjWait = new HBI.widget.Panel("wait", { width: "240px", fixedcenter: true, close: false, draggable: false, zindex:4, modal: true, visible: false } );
		...
		FormRegistro = new HBI.widget.Dialog("AluNew", { width : "68em", fixedcenter : true, visible : false, modal: true, constraintoviewport : true, 
									buttons : [ { text:"Guardar", handler:AluNew_Submit, isDefault:true }, { text:"Cancelar", handler:AluNew_Cancel } ]});
		FormRegistro.render();
		...
		MsjDelete = new HBI.widget.Dialog("RegDel", { width : "35em", fixedcenter : true, visible : false, modal: true, constraintoviewport : true, 
									buttons : [ { text:"Aceptar", handler:RegDel_Submit, isDefault:true }, { text:"Cancelar", handler:RegDel_Cancel } ]});
		MsjDelete.render();
